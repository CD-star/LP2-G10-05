...
public:
    Usuario(const string& user = "", const string& pass = "", const string& nombre = "",
            const string& dni = "", const string& placa = "")
...
int menu_usuario() {
    ...
    cout << "1. Actualizar mis datos" << endl; // Cambiado de "Ver Datos" y "Editar Datos" a una sola opcion mas completa
    cout << "2. Verificar Disponibilidad de Parqueadero" << endl;
    cout << "3. Ingresar mi Vehiculo" << endl;
    cout << "4. Salir mi Vehiculo y Pagar" << endl;
    cout << "5. Cerrar Sesion" << endl; // "Regresar" a "Cerrar Sesion" para mayor claridad
...
int menu_administrador() {
    ...
    cout << "6. Mostrar Estado Actual de Parqueaderos" << endl; // Nueva opcion para ver el estado de los parqueaderos
    cout << "7. Cerrar Sesion" << endl; // "Regresar" a "Cerrar Sesion"
    cout << "Ingrese una opcion: ";
    cin >> opc;
    return opc;
}
                    ...
                    while (opc_usuario_sub != 5) { // 5 es "Cerrar Sesion"
                        ...
                        try {
                            switch (opc_usuario_sub) {
                                case 1: { // Actualizar mis datos
                                    string nuevoNombre, nuevoDni, nuevaPlaca;
                                    cout << "Ingrese nuevo nombre completo (" << usuarioLogueado->getNombreCompleto() << "): "; getline(cin, nuevoNombre);
                                    if (nuevoNombre.empty()) nuevoNombre = usuarioLogueado->getNombreCompleto(); // Si esta vacio, mantiene el actual

                                    cout << "Ingrese nuevo DNI (" << usuarioLogueado->getDni() << "): "; getline(cin, nuevoDni);
                                    if (nuevoDni.empty()) nuevoDni = usuarioLogueado->getDni();

                                    cout << "Ingrese nueva placa de vehiculo asociada (" << usuarioLogueado->getPlacaVehiculoAsociada() << "): "; getline(cin, nuevaPlaca);
                                    if (nuevaPlaca.empty()) nuevaPlaca = usuarioLogueado->getPlacaVehiculoAsociada();
